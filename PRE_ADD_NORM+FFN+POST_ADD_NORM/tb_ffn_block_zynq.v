...
module tb_ffn_block_zynq;
    ...
    // -------------------------------------------------------------------
    ffn_block_zynq #(
        .D(D), .M(M), .NUM_COLS(NUM_COLS), .DATA_W(DATA_W), .FRAC_W(FRAC_W),
    ...
    reg signed [63:0] inter_wide [0:4*D-1];
    reg signed [63:0] relu_wide  [0:4*D-1];
    reg signed [63:0] ffn_wide   [0:D-1];
    // Q8.8 relu values (matching HW relu_bram storage)
    reg signed [DATA_W-1:0] relu_q8 [0:4*D-1];
    ...
    task compute_reference;
        ...
        begin
            ...
            $display("  Reference computed (D=%0d, 4D=%0d)", D, 4*D);
