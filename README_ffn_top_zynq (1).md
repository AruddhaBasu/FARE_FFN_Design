...
xc7z045ffv900-1. It uses aggressive time-multiplexing (`NUM_COLS=8`), a narrow
...
This configuration reduces all three to fit with comfortable margin.

## Key Design Decisions

### 1. NUM_COLS=8 (Time-Multiplexed Projection)

Each projection stage uses 8 parallel mul_col instances instead of 32.
This requires `ceil(32/8) = 4` sub-cycles per inner iteration.
...
| `M` | 32 | Tile dimension |
| `NUM_COLS` | 8 | Parallel columns per projection stage |
...
| NUM_SUB | 4 | ceil(32/8) — 4 sub-cycles per inner iteration |
...
## Resource Estimates (D=2048, M=32, NUM_COLS=8)

| Resource | Estimated | Available | Utilization |
|----------|-----------|-----------|-------------|
| DSP48E1 | 512 | 900 | 56.9% |
| LUT | ~50K | 218K | 23% |
| FF | ~30K | 437K | 7% |
| BRAM36 | ~30 | 545 | 6% |
| IOB | ~189 | 362 | 52% |
...
|-------|---------|-------------|-----|
| Up Projection | 8 | 8 × 32 | 256 |
| Down Projection | 8 | 8 × 32 | 256 |
| **Total** | | | **512** |

Remaining: 388 DSPs (43% margin)
...
### UP Phase (256 output tiles × 64 inner iterations × 4 sub-cycles)

```
For tile_col = 0 to 255:
  For inner_idx = 0 to 63:
    Fetch: input_tile[inner_idx] + W_up_tile[inner_idx][tile_col]
    Sub-cycle 0: up_proj(input_tile, W_up_tile[cols 0-7])
    Sub-cycle 1: up_proj(input_tile, W_up_tile[cols 8-15])
    Sub-cycle 2: up_proj(input_tile, W_up_tile[cols 16-23])
    Sub-cycle 3: up_proj(input_tile, W_up_tile[cols 24-31])
    → Accumulate into all 32 accumulator entries
  ReLU: max(0, result) → store in ReLU BRAM
```

### DOWN Phase (64 output tiles × 256 inner iterations × 4 sub-cycles)

```
For tile_col = 0 to 63:
  For inner_idx = 0 to 255:
    Fetch: relu_tile[inner_idx] + W_down_tile[inner_idx][tile_col]
    Sub-cycle 0: down_proj(relu_tile, W_down_tile[cols 0-7])
    Sub-cycle 1: down_proj(relu_tile, W_down_tile[cols 8-15])
    Sub-cycle 2: down_proj(relu_tile, W_down_tile[cols 16-23])
    Sub-cycle 3: down_proj(relu_tile, W_down_tile[cols 24-31])
    → Accumulate into all 32 accumulator entries
  Write result to output BRAM
```

### Sub-Cycle Detail (NUM_SUB=4)

```
Inner iteration with NUM_SUB=4:

  Cycle  0: Latch input + weight tile
  Cycle  1: Feed sub-cycle 0 (cols 0-7) → S_FEED
  Cycles 2-7: Adder tree pipeline + accumulate → S_PIPE → S_ACC
  Cycle  8: Feed sub-cycle 1 (cols 8-15) → S_FEED
  Cycles 9-14: Pipeline + accumulate
  Cycle 15: Feed sub-cycle 2 (cols 16-23) → S_FEED
  Cycles 16-21: Pipeline + accumulate
  Cycle 22: Feed sub-cycle 3 (cols 24-31) → S_FEED
  Cycles 23-28: Pipeline + accumulate
  (If is_last: output result; else: ready for next tile)
```

**Per inner iteration**: ~28 cycles (4 sub-cycles × ~7 cycles each + overhead)

**Total UP computation**: 256 × 64 × ~28 ≈ 458,752 cycles
**Total DOWN computation**: 64 × 256 × ~28 ≈ 458,752 cycles
**Estimated total**: ~918K cycles (+ AXI fetch overhead)

## Comparison with Other Configurations

| Metric | NUM_COLS=32 | NUM_COLS=16 | NUM_COLS=8 (Zynq) |
|--------|-------------|-------------|---------------------|
| DSP | 2,048 | 1,024 | **512** |
| LUT | ~150K | ~90K | **~50K** |
| IOB | 1,085 | 1,085 | **~189** |
...
| Sub-cycles/iter | 1 | 2 | 4 |
| Computation time | ~262K cyc | ~524K cyc | ~918K cyc |
...
rtl/tm_proj_stage.v     — Time-multiplexed projection (NUM_COLS=8, NUM_SUB=4)
